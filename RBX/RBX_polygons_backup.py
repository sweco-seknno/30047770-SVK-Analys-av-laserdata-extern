...
prj_dir = r"C:\Users\semnve\Desktop\RBX 2023-02-04"
# Path to the folder containing the RBX polygons.
shp_dir = os.path.join(prj_dir, "RBX_polygons")
# Path to the GeoDatabase.
gdb = os.path.join(prj_dir, "SVK_2023-02-04.gdb")
# Path to the line list of which the main function is based on.
LGs_info_path = os.path.join(prj_dir, "Ledningar_RBX_2023-02-03_A_LG028.txt")
...
def cluster_buffers(buffers):
    ...
    clustered_buffers = gpd.GeoDataFrame(buffers_union.explode())
    # Resetting indecies
    clustered_buffers.reset_index(inplace=True)
...
def read_points(gdb, LG, line):
    ...
    points = points[["AVSTAND_FAS", "AVSTAND_HORISONTELLT", "geometry"]]
...
def make_RBX_polygons(points, voltage, LG, littera, RBX_class):
    ...
    RBX_class_points = points[points["AVSTAND_FAS"] <= RBX_dist]
...
def set_class(points, voltage, LG, littera, RBX_class, RBX_class_nr):
    ...
    # Not edited.
    RBX_no_class_points = points[points["AVSTAND_FAS"] > RBX_dist]
...
def get_attributes_from_points(polygons, points):
    ...
    sorted_points = points_with_ID.sort_values(by='AVSTAND_FAS')
    ...
    polys = polys_[["LG", "littera", "AVSTAND_FAS",
                    "AVSTAND_HORISONTELLT", "geometry", "dz"]]
...
def set_z_to_points(points, raw_rbx_file):
    ...
    newpointlst["AVSTAND_HORISONTELLT"] = points["AVSTAND_HORISONTELLT"]
...
def round_values(gdf):
    ...
    rndvals = ['AVSTAND_FAS', 'AVSTAND_HORISONTELLT', 'dz']
    columns = gdf.columns
...
def RBX_polys_stats(row):
    ...
    points = read_points(gdb, LG, line)
    if str(type(points)) != "<class 'geopandas.geodataframe.GeoDataFrame'>":
    ...
    closest_points = closest_points.sort_values(by='AVSTAND_FAS')
    ...
    all_RBX_out.to_file(os.path.join(
        shp_dir, f"{LG}_{line}_RBX.shp"), crs="epsg:3006")
    ...
    all_closest_points_out.to_file(os.path.join(
        shp_dir, f"{LG}_{line}_closest_points.shp"), crs="epsg:3006")
...
create_dir(shp_dir)
LGs_info.apply(RBX_polys_stats, axis=1)
